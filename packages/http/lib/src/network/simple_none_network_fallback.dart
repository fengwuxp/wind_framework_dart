import 'dart:async';
import 'dart:io';

import 'package:wind_http/src/client/client_http_request.dart';
import 'package:wind_http/src/client/http_request_completer.dart';
import 'package:wind_http/src/network/none_network_fallback.dart';

/// Waiting for network readiness fail back
/// The current request is suspended when the network status is unavailable, waiting for a while, and the request is continued after the network is restored.
/// [WaitingNetworkReadinessFailBack.maxWaitTime]
/// [WaitingNetworkReadinessFailBack.maxWaitLength]
class WaitingNetworkReadinessFailBack<T extends ClientHttpRequest> implements NoneNetworkFallBack<T> {
  List<HttpRequestCompleter> _waitQueue = [];

  // max wait time millisecond
  final int maxWaitTime;

  // max wait queue length
  final int maxWaitLength;

  WaitingNetworkReadinessFailBack({this.maxWaitTime = 5 * 60 * 1000, this.maxWaitLength = 16});

  @override
  Future<T> onNetworkClose(T request) async {
    this._tryRemoveInvalidRequests();
    return this._addWaitRequest(request).future;
  }

  @override
  Future<void> onNetworkActive() async {
    if (_waitQueue.isEmpty) {
      return;
    }
    // copy References
    final copyQ = this._waitQueue;
    // 防止 onNetworkActive被多次调用
    this._waitQueue = [];
    copyQ.removeWhere((item) {
      item.completer.complete(item.request);
      return true;
    });
    // clear
    copyQ.clear();
  }

  Completer<T> _addWaitRequest(T request) {
    final waitQueue = this._waitQueue;
    if (waitQueue.length == maxWaitLength) {
      // 队列已满，强制移除掉第一个元素
      this._rejectHttpRequest(waitQueue.removeAt(0));
    }
    final completer = Completer<T>();
    this._waitQueue.add(HttpRequestCompleter(DateTime.now().millisecond + maxWaitTime, request, completer));
    return completer;
  }

  // 移除等待队列中无效的请求
  void _tryRemoveInvalidRequests() {
    final waitQueue = this._waitQueue, maxWaitLength = this.maxWaitLength;
    if (waitQueue.isEmpty) {
      return;
    }
    final currentTime = DateTime.now().millisecond;
    waitQueue.removeWhere((item) {
      // 是否过期
      final isExpire = item.isExpire(currentTime);
      if (isExpire) {
        this._rejectHttpRequest(item);
      }
      return isExpire;
    });
    final newLength = waitQueue.length;
    if (newLength < maxWaitLength || newLength == 0) {
      return;
    }
    // remove first item
    this._rejectHttpRequest(waitQueue.removeAt(0));
  }

  void _rejectHttpRequest(HttpRequestCompleter waitHttpRequest) {
    waitHttpRequest.completer.completeError(HttpException("token refresh failure", uri: waitHttpRequest.request.url));
  }
}
